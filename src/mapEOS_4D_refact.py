...
def getJacobian(local_TMUS, INTERP, absol_deriv=1e-3, deriv_percent=0.02):
    Jacobian = []
    dts = np.array([max(absol_deriv, local_quantity*deriv_percent) for local_quantity in local_TMUS])
...
def invert_EOS_tables(T_ID, Ttilde, iSt, iQt, iBt, TILDE_TABLES, TMU_TABLES, INTERP, MTP, guessSol=[0.05, 0.0, 0.0, 0.0]):
    ...
    #T_local, muB_local, muQ_local, muS_local, P_local, s_local = ed_local, nB_local, nQ_local, nS_local, ed_local*ns_local, nQ_local - 0.4 * nB_local
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Usage: python3 {} EOS_table_file Ttilde ID".format(sys.argv[0]))
        exit(1)
    ...
    # Akihiko's limit + reduced HRG Ttilde
    #Ttm, TtM, NTtilde = 0.024, 0.31, 10 
    TILDE_BOUNDARIES = [[0.024, 0.15, 5],     #T tilde
                            [-0.5, 0.5, 5],   #MUB tilde
                            [-0.2, 0.2, 5],   #MUQ tilde
                            [-0.15, 0.15, 5]] #MUS tilde
    ...
    with ProcessPoolExecutor(max_workers=N_CORES) as executor:
        ...
        for future in as_completed(future_to_x):  
            ...
            with open("RESULTS/TEMP_unordered_inversion_"+str(int(T_ID))+".dat", "a") as file:
